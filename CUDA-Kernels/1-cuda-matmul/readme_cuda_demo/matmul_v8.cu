...
__global__ void matmul_kernel_v8(float *dA, float *dB, float *dC, int M, int K, int N)
{
...
void matmul_cuda(float *hostA, float *hostB, float *hostC, int M, int K, int N){
    ...
    int repeat = 20;
    matmul_kernel_v8<BM, BN, BK, TM, TN><<<grid_dim, block_dim>>>(dA, dB, dC, M, K, N); // warm up
    ...
    for (int i = 0; i < repeat; i++) {
        matmul_kernel_v8<BM, BN, BK, TM, TN><<<grid_dim, block_dim>>>(dA, dB, dC, M, K, N);
    }
