...
__global__ void matmul_kernel_v5(float *dA, float *dB, float *dC, int M, int K, int N)
{
    __shared__ float SA[BM * BK]; // 128 * 8 = 1024
    __shared__ float SB[BK * BN]; // 8 * 128 = 1024
    int indA = TM * (blockIdx.x * blockDim.x); // blockIdx ： 在grid中的索引，全局x坐标
    int indB = TN * (blockIdx.y * blockDim.y); // 当前线程块的全局y坐标
    int width = (K + BK - 1) / BK;
    float tmp[TM * TN] = {0.0f};
    int tid = threadIdx.x + threadIdx.y * blockDim.x; // 线程索引: 0 ~ 256
    int smem_a_m = tid / 2;  // 0 ~ 127 行
    int smem_a_k = tid % 2;  // 0 ~ 1 列 --> 2 * 4 = 8 列
    int smem_b_k = tid / 32; // 0 ~ 7 行
    int smem_b_n = tid % 32; // 0 ~ 31 列 --> 32 * 4 = 128 列
    for (int ph = 0; ph < width; ph++)
    {
        // 256 个线程，每个线程加载4个数据，正好是1024个数据
        ...
        for (int index_q = 0; index_q < TM; index_q++)
        {
            for (int index_v = 0; index_v < TN; index_v++)
            {
                ...
                for (int index_k = 0; index_k < BK; index_k++) {   
                    // SA 访问 (8 * threadIdx.x + index_q)* 8 + index_k = 64 * threadIdx.x + k
                    // SB 访问 : 8 * threadIdx.y + k
                    // SA 存在严重的 bank conflict，导致性能下降。
...
void matmul_cuda_v5(float *hostA, float *hostB, float *hostC, int M, int K, int N)
{
    cudaSetDevice(3);
    double st, ela;
    st = get_walltime();
    ...
    int repeat = 20;
    matmul_kernel_v5<BM, BN, BK, TM, TN><<<grid_dim, block_dim>>>(dA, dB, dC, M, K, N); // warm up
    ...
    {
        matmul_kernel_v5<BM, BN, BK, TM, TN><<<grid_dim, block_dim>>>(dA, dB, dC, M, K, N);
    }
...
int main()
{
    ...
    matmul_cuda_v5(hostA, hostB, hostC, M, K, N);
    double st, ela;
    st = get_walltime();
